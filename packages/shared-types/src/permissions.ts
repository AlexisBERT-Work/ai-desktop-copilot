...
export const DEFAULT_PERMISSION_CONFIG: PermissionConfig = {
  ...
  tools: {
    ...
    suggest_refactor: { name: 'suggest_refactor', description: 'Detect refactoring opportunities (long functions, duplication, complexity) in a source file', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    review_diff: { name: 'review_diff', description: 'Review a git diff and surface likely issues (secrets, debug code, risky patterns)', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    analyze_dependencies: { name: 'analyze_dependencies', description: 'Parse package.json/Cargo.toml/requirements.txt and flag outdated or risky dependencies', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    ...
    bisect_guided: { name: 'bisect_guided', description: 'Plan a git bisect: count suspect commits, pick the next to test, and emit manual/automated commands', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    detect_spiral: { name: 'detect_spiral', description: 'Detect when the user is stuck looping on the same problem and suggest a break/new approach', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    generate_standup: { name: 'generate_standup', description: 'Draft a daily standup (yesterday/today/blockers) from recent git activity', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    summarize_git_log: { name: 'summarize_git_log', description: 'Summarize git history grouped by type/author/area over a time window or path', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    resolve_conflicts: { name: 'resolve_conflicts', description: 'Parse merge-conflicted files into ours/theirs blocks to propose a resolution', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    load_project_context: { name: 'load_project_context', description: 'Profile a project on open: stack, scripts, structure, entry points, README summary', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    ...
    get_market: { name: 'get_market', description: 'Lit l\'instantané bourse courant (cotations + formules calculées). Rafraîchit la watchlist', riskLevel: 'low', enabled: true, requiresConfirmation: false },
    add_to_watchlist: { name: 'add_to_watchlist', description: 'Ajoute un symbole à la watchlist bourse suivie en direct', riskLevel: 'medium', enabled: true, requiresConfirmation: false },
    remove_from_watchlist: { name: 'remove_from_watchlist', description: 'Retire un symbole de la watchlist bourse', riskLevel: 'medium', enabled: true, requiresConfirmation: false },
    set_formula: { name: 'set_formula', description: 'Crée ou modifie une formule mathématique recalculée en direct sur les cotations', riskLevel: 'medium', enabled: true, requiresConfirmation: false },
    remove_formula: { name: 'remove_formula', description: 'Supprime une formule de la watchlist bourse', riskLevel: 'medium', enabled: true, requiresConfirmation: false },
