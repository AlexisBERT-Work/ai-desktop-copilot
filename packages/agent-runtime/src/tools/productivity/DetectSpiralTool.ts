...
export function detectSpiral(events: ActivityEvent[], thresholdMinutes: number): SpiralVerdict {
  ...
  if (valid.length < 3) {
    return { spiraling: false, minutesOnTopic: 0, dominantSignature: null, repetitions: valid.length, failureCount: 0, reason: 'Pas assez d\'activité pour conclure.', suggestion: null };
  }
  ...
  const failureCount = onTopic.filter((e) => e.kind !== undefined && FAILURE_KINDS.has(e.kind)).length;

  // Spiral = same signature dominating, long enough, with repetition (and ideally failures).
  const spiraling =
    minutesOnTopic >= thresholdMinutes &&
    repetitions >= 3 &&
    onTopic.length / valid.length >= 0.5;
  ...
  if (spiraling) {
    ...
    reason = minutesOnTopic < thresholdMinutes
      ? `Sujet principal « ${dominant} » depuis ~${minutesOnTopic} min (< seuil ${thresholdMinutes}).`
      : 'Activité variée, pas de boucle dominante.';
  }

  return { spiraling, minutesOnTopic, dominantSignature: dominant || null, repetitions, failureCount, reason, suggestion };
}

export class DetectSpiralTool extends BaseTool {
  ...
  readonly schema = TOOL_SCHEMAS.detect_spiral;

  async execute(args: unknown): Promise<ToolResult> {
    const { events, threshold_minutes = 45 } = args as DetectSpiralArgs;
