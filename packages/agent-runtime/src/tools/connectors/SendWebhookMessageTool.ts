...
export function buildWebhookPayload(platform: Platform, message: string, username?: string): Record<string, unknown> {
  if (platform === 'slack') {
    return { text: message };
  }
  // discord
...
export class SendWebhookMessageTool extends BaseTool {
  ...
  readonly schema = TOOL_SCHEMAS.send_webhook_message;

  async execute(args: unknown): Promise<ToolResult> {
    const { message, platform = 'discord', username } = args as SendWebhookMessageArgs;

    if (typeof message !== 'string' || message.trim().length === 0) {
      return this.fail('message est requis et ne peut pas être vide.');
    }

    const url = resolveWebhookUrl(platform, (args as SendWebhookMessageArgs).webhook_url);
    if (url.length === 0) {
      return this.fail(`URL de webhook manquante. Passe webhook_url ou définis ${platform === 'slack' ? 'SLACK_WEBHOOK_URL' : 'DISCORD_WEBHOOK_URL'}.`);
    }
    if (!/^https:\/\//i.test(url)) {
