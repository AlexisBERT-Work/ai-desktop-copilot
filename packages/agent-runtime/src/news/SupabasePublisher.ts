...
export async function signIn(cfg: SupabaseAdminConfig): Promise<string> {
  ...
  const token =
    data !== null && typeof data === 'object' ? (data as Record<string, unknown>)['access_token'] : null;
...
async function insertDaily(cfg: SupabaseAdminConfig, jwt: string, draft: JournalDraft): Promise<void> {
...
export async function publishDailies(
  ...
  log.info('Dailies published', { published: result.published, skipped: result.skipped, errors: result.errors.length });
  return result;
}
